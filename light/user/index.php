...
try {
    ...
    $stmt_user = $db->prepare("SELECT tname FROM members WHERE Log_Id = ?");
    $stmt_user->execute([$Log_Id]);
    $user = $stmt_user->fetch();
    $mem_name = $user['tname'] ?? 'Guest_User';

    $db->beginTransaction();

    // 2. The SQL Query
    $sql = "INSERT INTO gym_logs (
                Log_Id, 
                mem_name, 
                exercise_name, 
                exercise_count, 
    ...
    $stmt = $db->prepare($sql);
    $stmt->execute([$Log_Id, $mem_name]);

    // --- CRITICAL STEP: UNCOMMENT THESE ---
    
    // Save the changes permanently
    $db->commit(); 
    
    // Clear the temporary table so it's ready for the next workout
    $db->exec("DELETE FROM gym_logsa");
								...
								<div class="card-body no-padding height-9">
									...
									<div class="table-responsive">
										<table
											...
											<tbody>
											<?php
												$result = $db->prepare("select * from  complaints where Log_Id='$Log_Id' limit 3");
												$result->execute();
							...
							<div class="card card-box">
								...
								<div class="card-body ">
									<div class="row">
										...
										<?php
											$result = $db->prepare("select * from  gym_logs where Log_Id='$Log_Id' order by ex_id  desc limit 2 ");
											$result->execute();
											for($i=0; $rows = $result->fetch(); $i++)
											{
