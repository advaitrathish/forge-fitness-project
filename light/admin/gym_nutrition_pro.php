                            ...
                            <div class="card card-box">
                                <div class="card-body">
                                    <form method="GET" action="" class="form-inline">
                                        <h4 class="mr-3">Intelligence Center:</h4>
                                        <select name="mem_name" class="form-control" onchange="this.form.submit()" style="min-width: 250px;">
                                            <option value="">-- Choose Member --</option>
                                            <?php
                                                $m_list = $db->query("SELECT DISTINCT mem_name FROM gym_logs ORDER BY mem_name ASC");
                                                while($m_row = $m_list->fetch()){
                                                    $s = ($filter_mem == $m_row['mem_name']) ? 'selected' : '';
                                                    echo "<option value='".$m_row['mem_name']."' $s>".$m_row['mem_name']."</option>";
                                                }
                                            ?>
                                        </select>
                                    </form>
